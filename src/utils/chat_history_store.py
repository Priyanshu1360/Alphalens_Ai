...
class ChatHistoryStore:
    ...
    def _local_timestamp(self, value: Optional[datetime]) -> Dict[str, Optional[str]]:
        if value is None:
            return {"created_at": None, "created_at_utc": None}
        ...
        return {
            "created_at": local_value.strftime("%Y-%m-%d %H:%M:%S %Z"),
            "created_at_utc": utc_value.isoformat(),
        }

    def _sort_thread_ids(self, thread_ids: List[str]) -> List[str]:
        def sort_key(thread_id: str):
            text = str(thread_id).strip()
            return (0, int(text)) if text.isdigit() else (1, text.lower())

        return sorted(thread_ids, key=sort_key)
        ...
        return [
            {
                ...
                **self._local_timestamp(row.created_at),
            }
            for row in rows
        ]
    ...
    def list_thread_ids(self) -> List[str]:
        with self.SessionLocal() as session:
            rows = session.execute(select(ChatHistoryRecord.thread_id).distinct()).all()
        thread_ids = [
            str(row[0]).strip()
            for row in rows
            if row[0] is not None and str(row[0]).strip()
        ]
        return self._sort_thread_ids(thread_ids)

    def next_numeric_thread_id(self) -> str:
        max_id = 0
        for thread_id in self.list_thread_ids():
            if thread_id.isdigit():
                max_id = max(max_id, int(thread_id))
        return str(max_id + 1)
