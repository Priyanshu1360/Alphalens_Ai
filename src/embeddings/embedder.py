...
_SPARSE_BM25_STATS = None


def _normalize_sparse_stats_path():
    raw_path = str(Config.SPARSE_BM25_STATS_PATH or "").strip()
    if not raw_path:
        raw_path = "Data/bm25_sparse_stats.json"
...
def _tokenize_sparse(text):
    return SPARSE_TOKEN_PATTERN.findall((text or "").lower())


def _hashed_sparse_index(token):
    digest = hashlib.sha1(token.encode("utf-8")).digest()
    return int.from_bytes(digest[:4], "big") % Config.SPARSE_HASH_SPACE


def _build_bm25_statistics(texts):
    doc_count = 0
    total_length = 0
    index_doc_freq = Counter()
    ...
    for text in texts:
        ...
        seen_indices = set()
        for token in tokens:
            index = _hashed_sparse_index(token)
            if index in seen_indices:
                continue
            seen_indices.add(index)
            index_doc_freq[index] += 1

    avg_doc_len = total_length / doc_count if doc_count else 0.0
    return {
        "doc_count": doc_count,
        "avg_doc_len": avg_doc_len,
        "index_doc_freq": {str(index): freq for index, freq in index_doc_freq.items()},
    }


def _save_sparse_stats(stats):
    global _SPARSE_BM25_STATS

    _SPARSE_BM25_STATS = dict(stats or {})
    path = _normalize_sparse_stats_path()
    try:
        path.parent.mkdir(parents=True, exist_ok=True)
        with path.open("w", encoding="utf-8") as handle:
            json.dump(_SPARSE_BM25_STATS, handle)
    except Exception:
        pass


def _load_sparse_stats():
    global _SPARSE_BM25_STATS

    if _SPARSE_BM25_STATS is not None:
        return _SPARSE_BM25_STATS
...
def _bm25_weight(term_frequency, document_frequency, doc_count, doc_length, avg_doc_length):
    if term_frequency <= 0 or document_frequency <= 0 or doc_count <= 0:
        return 0.0

    avg_doc_length = max(float(avg_doc_length or 0.0), 1e-9)
    doc_length = max(float(doc_length or 0.0), 1.0)
    k1 = max(float(Config.SPARSE_BM25_K1), 0.0)
    b = min(max(float(Config.SPARSE_BM25_B), 0.0), 1.0)
...
def _build_sparse_vector(text, sparse_stats=None):
    tokens = _tokenize_sparse(text)
    if not tokens:
        return SparseVector(indices=[], values=[])

    if str(Config.SPARSE_BACKEND or "").strip().lower() != "bm25":
    ...
    if sparse_stats is None:
        sparse_stats = _load_sparse_stats()
    ...
    counts = Counter(tokens)
    hashed = {}
    doc_count = int(sparse_stats.get("doc_count") or 0)
    avg_doc_len = float(sparse_stats.get("avg_doc_len") or 0.0)
    doc_length = len(tokens)
    index_doc_freq = {
        int(index): int(freq)
        for index, freq in (sparse_stats.get("index_doc_freq") or {}).items()
    }

    for token, count in counts.items():
        index = _hashed_sparse_index(token)
        document_frequency = index_doc_freq.get(index, 0)
        weight = _bm25_weight(
        ...
        if weight > 0:
            hashed[index] = hashed.get(index, 0.0) + weight
...
def store_embeddings(chunks, embeddings, batch_size=None, sparse_stats=None):
    ...
    try:
        for chunk_batch, embedding_batch in zip(
            ...
            for chunk, embedding in zip(chunk_batch, embedding_batch):
                ...
                sparse_vector = _build_sparse_vector(
                    payload.get("text", ""),
...
def embed_and_store_chunks(chunks):
    ...
    sparse_stats = _build_bm25_statistics(texts)
    _save_sparse_stats(sparse_stats)
    embeddings = embed_texts(texts)
    inserted_count = store_embeddings(chunks, embeddings, sparse_stats=sparse_stats)
    return inserted_count
