...
from src.agent_graph import AgentWorkflow
from src.cache_memory import ExactMatchCache, SemanticCache
from src.config import Config
from src.ingestion_service import ingest_all_documents, ingest_single_pdf
from src.mcp_client import MCPClient
from src.rag_service import default_mode, run_rag_query
...
semantic_cache = (
    SemanticCache(
        ...
        similarity_threshold=float(
            os.getenv("SEMANTIC_CACHE_SIMILARITY_THRESHOLD", "0.9")
        ),
    )
...
agent_workflow = AgentWorkflow(cache=exact_cache, mcp_client=mcp_client)
...
def query(payload: QueryRequest) -> Dict[str, Any]:
    ...
    if semantic_cache is not None:
        semantic_cache.set(
            query_text,
            {"answer": result.get("answer", ""), "docs": result.get("results", [])},
        )
